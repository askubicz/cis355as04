...
if ( !empty($_POST)) { // if $_POST filled then process the form
	...
	# initialize/validate (same as file: as04_per_create.php)
	...
	if ($valid) { // if valid user input update the database
		...
		if($fileSize > 0) { // if file was updated, update all fields
			...
			$sql = "UPDATE as04_persons  set fname = ?, lname = ?, email = ?, mobile = ?, password = ?, title = ?, filename = ?,filesize = ?,filetype = ?,filecontent = ? WHERE id = ?";
			$q = $pdo->prepare($sql);
			$q->execute(array($fname, $lname, $email, $mobile, $password, $title, $fileName,$fileSize,$fileType,$content, $id));
			Database::disconnect();
			header("Location: as04_persons.php");
		}
		else { // otherwise, update all fields EXCEPT file fields
			$pdo = Database::connect();
			$pdo->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
			$sql = "UPDATE as04_persons  set fname = ?, lname = ?, email = ?, mobile = ?, password = ?, title = ? WHERE id = ?";
			$q = $pdo->prepare($sql);
	...
	$pdo = Database::connect();
	$pdo->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
	$sql = "SELECT * FROM as04_persons where id = ?";
	$q = $pdo->prepare($sql);
	$q->execute(array($id));
...
<body>
    <div class="container">
		...
		<div class="span10 offset1">
			...
			<form class="form-horizontal" action="as04_per_update.php?id=<?php echo $id?>" method="post" enctype="multipart/form-data">
				...
				<div class="control-group">
					...
					<div class="controls">
						<select class="form-control" name="title">
							...
							if (0==strcmp($_SESSION['as04_person_title'],'Volunteer')) echo '<option selected value="Volunteer" >Volunteer</option>';
				...
				<div class="form-actions">
					...
					<a class="btn" href="as04_persons.php">Back</a>
				</div>
